...
from app.module_users.models import AchievementProgress, BannedUsers, FacebookAuth, Friend, FriendInvite, GoogleAuth, User, ViajuntosAuth, UserLanguage
...
def get_reported_events():
    ...
    reported_events = ReportedEvent.query.all().order_by(desc(ReportedEvent.date))
...
def get_reported_users():
    ...
    user_ids = [(reported_user.id_user, reported_user.id_user_reported) for reported_user in reported_users]
    all_users = User.query.filter(User.id.in_(user_ids)).all()
    user_map = {(user.id, user.id_user_reported): user for user in all_users}
...
@module_admin_v1.route('/banned_user', methods=['GET'])
@jwt_required(optional=False)
def get_banned_user():
    auth_id = get_jwt_identity()
    if not Admin.exists(auth_id):
...
@module_admin_v1.route('/banned_event', methods=['GET'])
@jwt_required(optional=False)
def get_banned_event():
    auth_id = get_jwt_identity()
    if not Admin.exists(auth_id):
        return jsonify({'error_message': 'Only administrators can make this action.'}), 403
    banned_events = Event.query.filter_by(banned = True).all()
...
def ban_user():
    ...
    # if status != 202:
    #     return jsonify({'error_message': 'Messages cannot be successfully deleted.', 'details': msg['error_message']}), 500
    ...
    if BannedUsers.exists(email_to_ban):
        return jsonify({'error_message': 'User email already banned.'}), 200
    ban_instance = BannedUsers(email_to_ban, username_to_ban, current_time, ban_reason,auth_id)
    ban_instance.save()
